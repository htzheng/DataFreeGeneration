...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, group_norm=0):
        ...
        for m in self.modules():
            ...
            #     m.weight.data.fill_(1)
            #     m.bias.data.zero_()
            # elif isinstance(m, InstanceNorm2d): # zht: because instance norm doesn't have weight, we dont do init
            #     m.weight.data.fill_(1)
            #     m.bias.data.zero_()

        # zht: because instance norm doesn't have weight, we dont do init
        # for m in self.modules():
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        missing_keys, unexpected_keys = model.load_state_dict(model_zoo.load_url(model_urls['resnet50']), strict=False)   # load from pretrained batch normalization model for finetunning
