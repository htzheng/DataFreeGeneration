...
def main_worker(gpu, ngpus_per_node, args):
    ...
    #     torchvision.models.resnet50(pretrained=True, progress=True)
    # else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, max_iter=100)

        # evaluate on validation set
        acc1 = validate(val_loader, model, criterion, args, max_iter=100)
...
def train(train_loader, model, criterion, optimizer, epoch, args, max_iter=None):
...
def validate(val_loader, model, criterion, args, max_iter=None):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
...
def save_checkpoint_simple(state, is_best, filename='checkpoint.pth', root='./'):
    torch.save(state, os.path.join(root, filename))
    if is_best:
        shutil.copyfile(os.path.join(root,filename), os.path.join(root,'model_best.pth'))
